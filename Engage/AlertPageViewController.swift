...
//            pageContentViewController = UINib(nibName: "AlertChildPageViewController", bundle: podBundle).instantiate(withOwner: nil, options: nil)[0] as! AlertChildPageViewController
            
            pageContentViewController = podBundle.loadNibNamed("AlertChildPageViewController", owner: self, options: nil)?[0] as! AlertChildPageViewController
        }
        ...
        pageContentViewController.image.image = getImage(imageUrl: arrayOfImage[realIndex])
    ...
    internal func getImage(imageUrl: String) -> UIImage{
        var imageData:Data
        do {
            if(imageUrl != ""){
